#!/usr/bin/env python3
"""Compute flood risk R = (H × E × V)^(1/3) for a city.

Rasterizes ACS block-group exposure/vulnerability onto the flood hazard grid,
...
def run(site: str, root: Path | None = None) -> Path:
    ...
    # Geometric mean — same as poa_flood_risk
    ...
    prefix = site
    risk_tif = out_dir / f"flood_risk_score_{prefix}.tif"
    e_tif = out_dir / f"flood_exposure_score_{prefix}.tif"
    v_tif = out_dir / f"flood_vulnerability_score_{prefix}.tif"
    write_geotiff(risk_tif, risk, profile, "flood_risk_geometric_mean_HxExV")
    ...
    zonal = zonal_means(
        ...
        {
            "flood_hazard_mean": h,
            "flood_exposure_score": e,
            "flood_vulnerability_score": v,
    ...
    gpkg = out_dir / f"flood_risk_score_{prefix}.gpkg"
    geojson = out_dir / f"flood_risk_score_{prefix}.geojson"
    ...
    write_choropleth_svg(
        zonal_out,
        "flood_risk_score",
        out_dir / "map_flood_risk_score.svg",
        title=f"Flood risk R=(H×E×V)^(1/3) — {display}",
        subtitle="Block-group mean · H=flood IDW · E/V=ACS",
    )
    ...
    write_raster_grid_svg(
        risk,
        out_dir / "map_flood_risk_score_grid.svg",
        title=f"Flood risk grid R=(H×E×V)^(1/3) — {display}",
        subtitle=grid_sub,
        vmin=0.0,
        vmax=1.0,
    )
    write_raster_grid_svg(
        h,
        out_dir / "map_flood_hazard_grid.svg",
        title=f"Flood hazard H (IDW) — {display}",
        subtitle=grid_sub,
        vmin=0.0,
        vmax=1.0,
    )
    write_raster_grid_svg(
        e,
        out_dir / "map_flood_exposure_grid.svg",
    ...
    write_raster_grid_svg(
        v,
        out_dir / "map_flood_vulnerability_grid.svg",
    ...
    meta = {
        ...
        "hazard": {
            ...
            "role": "H",
            "description": "Flood hazard IDW score (0–1)",
        },
        "exposure": {
            "path": str(acs_path),
            "field": e_field,
        ...
        "sector": {
            "sector": "Hydrogeological Disasters",
            "risk": "Floods",
            "note": "Shared ACS E/V also apply to heat/landslide risk when those hazards are combined the same way.",
        },
        "outputs": {
