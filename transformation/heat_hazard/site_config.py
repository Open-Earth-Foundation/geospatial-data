...
def find_repo_root(heat_hazard_root: Path) -> Path:
    """Resolve geospatial-data root from transformation/heat_hazard."""
    return Path(heat_hazard_root).resolve().parent.parent


def model_config_path(heat_hazard_root: Path | None = None) -> Path:
    root = Path(heat_hazard_root or find_heat_hazard_root()).resolve()
    return find_repo_root(root) / "models" / "heat_hazard" / "config.yaml"


def load_model_config(heat_hazard_root: Path | None = None) -> dict[str, Any]:
    """Load default heat_hazard model parameters from models/heat_hazard/config.yaml."""
    path = model_config_path(heat_hazard_root)
    if not path.exists():
        return {}
...
def load_site_config(site_slug: str | None = None, root: Path | None = None) -> dict[str, Any]:
    ...
    Model defaults from ``models/heat_hazard/config.yaml`` are merged under
    ``hazard`` and ``publish``; city YAML values win on conflict.
    """
    ...
    model = load_model_config(heat_hazard_root)
    model_path = model_config_path(heat_hazard_root)
    ...
    gpkg = bairro.get("gpkg_path") or os.environ.get("HEAT_BAIRRO_GPKG")
