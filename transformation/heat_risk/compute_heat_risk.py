#!/usr/bin/env python3
"""Compute heat risk R = (H × E × V)^(1/3) for a city.

Rasterizes ACS block-group exposure/vulnerability onto the heat hazard grid,
...
def run(site: str, root: Path | None = None) -> Path:
    ...
    # Geometric mean — same as poa_heat_risk
    ...
    write_geotiff(risk_tif, risk, profile, "heat_risk_geometric_mean_HxExV")
    ...
    zonal = zonal_means(
        ...
        {
            "heat_hazard_mean": h,
            "heat_exposure_score": e,
            "heat_vulnerability_score": v,
    ...
    gpkg = out_dir / f"heat_risk_score_{prefix}.gpkg"
    geojson = out_dir / f"heat_risk_score_{prefix}.geojson"
    ...
    meta = {
        ...
        "hazard": {
            ...
            "role": "H",
            "description": "Heat hazard ensemble score (0–1)",
        },
        "exposure": {
            "path": str(acs_path),
            "field": e_field,
