#!/usr/bin/env python3
"""Compute grid-cell landslide mechanism type for a city (NBS screening L2).

Screens each hazard-valid 90 m cell inside the city boundary, classifies the
dominant landslide mechanism, IDW-fills gaps, and writes GeoTIFF + GeoJSON exports.

Example:
  python transformation/nbs_screening/landslide/compute_mechanism.py --site richfield
  python transformation/nbs_screening/landslide/compute_mechanism.py --site porto_alegre --aoi full
  python transformation/nbs_screening/check_nbs_layers.py --site richfield --hazard landslide
"""
...
from grid_screening import (  # noqa: E402
    ...
    export_landslide_mechanism_layers,
    landslide_mechanism_layer_stem,
    result_to_geojson,
    result_to_report_dict,
    screen_site_landslide_mechanism_grid,
...
LANDSLIDE_MECHANISM_CODE_COLORS: dict[int, str] = {
    code: LANDSLIDE_MECHANISM_COLORS[mech_type]
    for mech_type, code in LANDSLIDE_MECHANISM_TYPE_CODES.items()
}
NODATA_FILL = "#eeeeee"


def write_landslide_mechanism_grid_svg(
    raster_path: Path,
    out_path: Path,
    *,
    title: str,
    ...
    for i in range(nrows):
        for j in range(ncols):
            ...
            fill = (
                ...
                else LANDSLIDE_MECHANISM_CODE_COLORS.get(code, "#cccccc")
            )
            x = margin_l + j * cell_w
            y = margin_t + i * cell_h
            parts.append(
    ...
    legend_rows = [
        ...
        *[
            (
                code,
                LANDSLIDE_MECHANISM_CODE_COLORS[code],
                f"{code} · {LANDSLIDE_MECHANISM_CLASS_NAMES[code]}",
            )
            for code in sorted(LANDSLIDE_MECHANISM_CODE_COLORS)
        ],
    ]
...
def model_config_path() -> Path:
    return find_repo_root(NBS_ROOT) / "models" / "nbs_landslide_mechanism_type" / "config.yaml"
...
def _require_layers(site: str) -> None:
    sources = get_layer_sources("landslide", site)
    missing = [layer for layer in HAZARD_REQUIRED_LAYERS["landslide"] if layer not in sources]
    if missing:
        raise FileNotFoundError(
            f"Missing required landslide layers for site={site}: {missing}. "
            f"Run: python transformation/nbs_screening/check_nbs_layers.py "
            f"--site {site} --hazard landslide"
        )


def _resolve_aoi(site: str, aoi_mode: str):
    if aoi_mode == "full":
        return site_reference_bounds_geom("landslide", site)
    if aoi_mode == "boundary":
        return None
    raise ValueError(f"Unknown --aoi mode: {aoi_mode}")


def compute_landslide_mechanism(
    site: str,
    *,
    aoi: str = "boundary",
    ...
    ref_path = get_reference_hazard_raster("landslide", site)
    out_dir = Path(out_dir or site_output_dir(site, "landslide"))
    out_dir.mkdir(parents=True, exist_ok=True)

    aoi_geom = _resolve_aoi(site, aoi)
    print(f"Screening landslide mechanism grid for {site_cfg.get('display_name', site)} ({site})")
    print(f"  reference hazard: {ref_path}")
    print(f"  AOI mode: {aoi}")
    print(f"  output: {out_dir}")

    result = screen_site_landslide_mechanism_grid(
        site,
        aoi_geom=aoi_geom,
    ...
    raster_paths = export_landslide_mechanism_layers(result, out_dir, site=site)
    stem = landslide_mechanism_layer_stem(site)
    geojson_path = out_dir / f"{stem}.geojson"
    report_path = out_dir / f"nbs_grid_landslide_{site}.json"

    geojson_payload = result_to_geojson(result)
    geojson_payload["properties"]["layer"] = (
        "poa_landslide_mechanism_type"
        if site == DEFAULT_SITE
        else f"{site}_landslide_mechanism_type"
    )
    ...
    if write_qa:
        ...
        write_landslide_mechanism_grid_svg(
            filled_path,
            qa_map_path,
            title=f"Landslide mechanism type (IDW-filled) — {display}",
            subtitle=f"90 m grid · {filled_path.name}",
        )
    ...
    meta = {
        ...
        "hazard": "landslide",
        "layer_id": model_cfg.get("layer_id", "nbs_landslide_mechanism_type"),
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--out-dir",
        default=None,
        help="Override output directory (default: landslide/sites/<site>/data/output)",
    )
    parser.add_argument(
        "--no-qa",
        action="store_true",
