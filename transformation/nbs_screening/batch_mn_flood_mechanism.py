#!/usr/bin/env python3
"""Batch flood mechanism pipeline for Minnesota cities (N5).

Runs per site: OSM waterways extract (optional) → grid compute → COG/tiles publish.

Example (local build only, skip cities that fail):
  python transformation/nbs_screening/batch_mn_flood_mechanism.py

Example (extract rivers when missing, upload + catalog):
  python transformation/nbs_screening/batch_mn_flood_mechanism.py \\
    --extract-rivers --upload --write-catalog
...
from site_config import MN_SITES, load_site_config, resolve_osm_rivers_path  # noqa: E402
...
def _parse_sites(raw: str | None) -> list[str]:
    if not raw:
        return list(MN_SITES)
    sites = [s.strip() for s in raw.split(",") if s.strip()]
    unknown = [s for s in sites if s not in MN_SITES]
    if unknown:
        raise ValueError(f"Unknown site(s): {unknown}. Choose from: {', '.join(MN_SITES)}")
    return sites
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--sites",
        help=f"Comma-separated city slugs (default: all MN: {', '.join(MN_SITES)})",
    )
    parser.add_argument(
        "--extract-rivers",
        action="store_true",
