#!/usr/bin/env python3
"""Build heat mechanism COG + XYZ tiles, upload to S3, upsert catalog/datasets.yaml.
...
Example (upload + catalog):
  python transformation/nbs_screening/heat/publish_mechanism.py \\
...
from grid_screening import heat_mechanism_layer_stem  # noqa: E402
...
MAX_CODE = max(HEAT_MECHANISM_TYPE_CODES.values())


def write_heat_mechanism_colors(colors_path: Path) -> None:
    write_mechanism_colors(
        colors_path,
        header_lines=[
            "# Dominant heat mechanism type. GDAL color-relief for visual tiles.",
            f"# Raster nodata = {MECHANISM_RASTER_NODATA}; code 0 = without_clear_dominant",
        ],
        type_codes=HEAT_MECHANISM_TYPE_CODES,
        type_colors=HEAT_MECHANISM_COLORS,
    )


def resolve_input_tif(site: str) -> Path:
    stem = heat_mechanism_layer_stem(site)
    path = resolve_site_output_dir(site, "heat") / f"{stem}.tif"
    if not path.is_file():
        raise FileNotFoundError(
            f"Missing filled mechanism raster: {path}. "
            f"Run: python transformation/nbs_screening/heat/compute_mechanism.py --site {site}"
        )
    return path
...
def resolve_geojson(site: str) -> Path:
    stem = heat_mechanism_layer_stem(site)
    path = resolve_site_output_dir(site, "heat") / f"{stem}.geojson"
    if not path.is_file():
        raise FileNotFoundError(
            f"Missing mechanism GeoJSON: {path}. "
            f"Run heat/compute_mechanism.py --site {site} first."
        )
    return path
...
def build_catalog_entry(
    ...
    *,
    ...
    dataset_id = dataset_id_for_site(site_slug, hazard="heat")
    slug = heat_mechanism_layer_stem(site_slug)
    hazard_id = "poa_heat_hazard" if site_slug == DEFAULT_SITE else f"{site_slug}_heat_hazard"

    classes: dict[int, dict[str, str]] = {}
    for mech_type, code in sorted(HEAT_MECHANISM_TYPE_CODES.items(), key=lambda kv: kv[1]):
        classes[code] = {
            "name": HEAT_MECHANISM_CLASS_NAMES[code],
            "color": HEAT_MECHANISM_COLORS[mech_type],
        }

    return {
        "dataset_id": dataset_id,
        "dataset_name": f"Heat Mechanism Type ({short}, 250 m)",
        ...
        "source_url": hazard_source_url(
            ...
            default_hazard_key="heat/hazard/heat_hazard_score_idw_cog.tif",
            catalog_path=catalog_path,
        ),
        "dataset_type": "heat",
        "type": "categorical_raster",
        "data_quality": {
            "temporal_coverage": "Static screening layer from current catalog COGs and LST inputs",
            "accuracy": (
                "Per-cell dominant mechanism from rule-based NBS screening "
                "(UHI, shade deficit, daytime LST, nocturnal cooling, social exposure)"
            ),
            "limitations": (
                "Screening-level classification only; LST proxies are surface temperature, "
                "not pedestrian thermal comfort"
            ),
        },
        "value_encoding": {
            "type": "class_lookup",
        ...
        "description": (
            f"OEF dominant heat mechanism type per 250 m cell for {display}. Integer codes 0–6 "
            "for without_clear_dominant, uhi_built_up, shade_deficit, high_daytime_lst, "
            "limited_nocturnal_cooling, high_social_exposure, mixed. Rules in "
            "`transformation/nbs_screening/nbs_rules.py`; produced by "
            "`transformation/nbs_screening/heat/compute_mechanism.py`. Methodology: "
            "`models/nbs_heat_mechanism_type/`. Rasterized on the OEF heat hazard 250 m grid."
        ),
        "_slug": slug,
    }
...
def run_publish(
    ...
    *,
    ...
    site_slug = str(site_cfg["site_slug"])
    slug = heat_mechanism_layer_stem(site_slug)
    publish_dir = site_publish_dir(site_slug, "heat")
    s3_prefix = s3_prefix_for_site(site_slug, hazard_subdir="heat/heat_mechanism")
    zoom = tile_zoom or DEFAULT_TILE_ZOOM

    if build:
        in_tif = resolve_input_tif(site_slug)
        print(f"Building heat mechanism publish artifacts from {in_tif}")
        build_mechanism_cog_and_tiles(
            in_tif=in_tif,
            publish_dir=publish_dir,
            slug=slug,
            write_colors=write_heat_mechanism_colors,
            max_code=MAX_CODE,
            decode_title=f"Heat mechanism type value tiles ({slug})",
            decode_code_name="heat_mechanism_code",
            type_codes=HEAT_MECHANISM_TYPE_CODES,
            tile_zoom=zoom,
        )
    else:
