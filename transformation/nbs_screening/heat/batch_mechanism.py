#!/usr/bin/env python3
"""Batch heat mechanism pipeline for configured NBS screening cities (H4).

Per site: mechanism input extract (optional) → grid compute → COG/tiles publish.

Example:
  python transformation/nbs_screening/heat/batch_mechanism.py --site richfield
  python transformation/nbs_screening/heat/batch_mechanism.py --country "United States"
  python transformation/nbs_screening/heat/batch_mechanism.py \\
...
from compute_mechanism import compute_heat_mechanism  # noqa: E402
...
def _run_inputs_extract(site: str, *, dry_run: bool) -> tuple[StepStatus, str | None]:
    cmd = [
        runner_python(),
        str(HEAT_ROOT / "extract_mechanism_inputs.py"),
        "--site",
        site,
    ]
    if dry_run:
...
def run_site_pipeline(
    ...
    *,
    ...
    if skip_compute:
        ...
        print(f"  [dry-run] compute_heat_mechanism --site {site}")
        result.steps["compute"] = "skipped"
    else:
        try:
            compute_heat_mechanism(site, aoi="boundary")
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--extract-inputs",
        action="store_true",
        help="Always run heat/extract_mechanism_inputs.py before compute",
    )
    parser.add_argument(
        "--extract-if-missing",
        action="store_true",
    ...
    print(f"Heat pipeline sites ({len(sites)}): {', '.join(sites)}")
    results = run_batch(
        sites,
