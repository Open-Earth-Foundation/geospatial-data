"""Shared helpers for Hansen forest change site extract CLIs."""
...
REPO_ROOT = HANSEN_ROOT.parent.parent
FLOOD_HAZARD_ROOT = HANSEN_ROOT.parent / "flood_hazard"
NBS_ROOT = HANSEN_ROOT.parent / "nbs_screening"

CRS = "EPSG:4326"
SCALE_M = 30
EE_IMAGE = "UMD/hansen/global_forest_change_2024_v1_12"
EE_BAND = "treecover2000"
...
def treecover_output_path(site: str, *, prefix: str) -> Path:
    return (
        HANSEN_ROOT
        / "sites"
        / site
        / "data"
        / "output"
        / f"{prefix}_hansen_treecover2000_30m.tif"
    )


def hansen_qa_dir(site: str) -> Path:
    return HANSEN_ROOT / "sites" / site / "data" / "intermediate" / "qa_inputs"


def load_hansen_site(site: str | None = None) -> dict[str, Any]:
    ...
    out_path = treecover_output_path(slug, prefix=prefix)
    qa_dir = hansen_qa_dir(slug)
    return {
        **flood_cfg,
        "hansen_output_path": out_path,
        "hansen_output_path_abs": str(out_path.resolve()),
        "hansen_qa_dir_abs": str(qa_dir.resolve()),
    }
