#!/usr/bin/env python3
"""Export JRC Global Surface Water layers for NBS flood mechanism screening.

Writes NBS catalog rasters under ``jrc_global_surface_water/sites/{site}/data/output/``:

  - ``{prefix}_gsw_occurrence_30m.tif`` — water occurrence (%)
  - ``{prefix}_gsw_seasonality_30m.tif`` — months water present (0–12)
  - ``{prefix}_gsw_transition_30m.tif`` — transition class (0–10)

Source: GEE ``JRC/GSW1_4/GlobalSurfaceWater``.
...
if str(GSW_ROOT) not in sys.path:
    sys.path.insert(0, str(GSW_ROOT))
...
from qa_gsw import QA_BY_LAYER  # noqa: E402

DEFAULT_ORDER: tuple[GswLayer, ...] = GSW_LAYERS
...
def _export_band(
    ...
    *,
    ...
    img = gsw.select(band).clip(roi).reproject(crs=CRS, scale=SCALE_M).toByte()
...
def run(
    site: str,
    *,
    layers: tuple[GswLayer, ...] = DEFAULT_ORDER,
    authenticate: bool = False,
    write_qa: bool = True,
    qa_only: bool = False,
    dry_run: bool = False,
) -> SiteRunResult:
    cfg = load_gsw_site(site)
    display = str(cfg.get("display_name") or site)
    paths = {layer: Path(cfg["gsw_paths"][layer]) for layer in GSW_LAYERS}
    ...
    if not qa_only:
        ...
        gsw = ee.Image(EE_IMAGE)
        print(f"JRC GSW → {display} ({site}) · layers: {', '.join(layers)}")

    for layer in layers:
        out = paths[layer]
        if not qa_only:
            _export_band(gsw, roi, band=layer, out_path=out)
    ...
    if write_qa and not dry_run:
        print(f"QA SVGs: {cfg['gsw_qa_dir_abs']}")
...
def _parse_layers(raw: str | None) -> tuple[GswLayer, ...]:
    if not raw:
        return DEFAULT_ORDER
    keys = [k.strip() for k in raw.split(",") if k.strip()]
    valid = set(GSW_LAYERS)
    unknown = [k for k in keys if k not in valid]
    if unknown:
        raise ValueError(f"Unknown layers: {unknown}. Valid: {', '.join(GSW_LAYERS)}")
    return tuple(keys)  # type: ignore[return-value]


def _print_summary(results: list[SiteRunResult]) -> None:
    print("\n=== JRC GSW export summary ===")
    for row in results:
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--only",
        default=None,
        help=f"Comma-separated subset of: {','.join(GSW_LAYERS)}",
    )
